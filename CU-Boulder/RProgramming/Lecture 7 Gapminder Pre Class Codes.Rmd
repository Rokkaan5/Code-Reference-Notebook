...
---
title: 'Data Visualization Case Study: Trends in World Health and Economics'
...
#### We prefer labels over legends (this will be on the quiz)
...
gapminder %>% filter(year %in% c(past_year, present_year) & country %in% country_list) %>% 
  ...
  select(country, region, continent, year, dollars_per_day) %>% head()
...
gapminder %>% 
  ...
  select(country, region, continent, year, dollars_per_day) %>%
  spread(year, dollars_per_day)  %>%
  mutate(difference = present/past) %>%      # "difference" is a ratio in this case
...
gapminder %>% 
  ...
  summarize(n=n()) %>% knitr::kable()   # puts all tables into one grid (apparently)
```
